...
Vendored from github.com/grip-unina/TruFor (test_docker/src/models/cmx/,
commit at vendoring time, see ../../LICENSE.txt), byte-for-byte except for
one line below: upstream's package is named `models`, which collides with
this repo's own document-forensics/.gitignore `models/` rule (meant for
downloaded model caches), so it's renamed `trufor_model` on this side only.
"""
...
class myEncoderDecoder(nn.Module):
    def __init__(self, cfg=None, norm_layer=nn.BatchNorm2d):
        ...
        from trufor_model.DnCNN import make_net  # renamed from `models.DnCNN`, see module docstring
        num_levels = 17
        out_channel = 1
