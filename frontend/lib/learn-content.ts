...
/**
 * A round of up to ROUND_SIZE questions with a few genuine messages mixed in
 * (so "always scam" doesn't win). Without `random` the round is
 * deterministic, so the first play is the same every time (rehearsable for
 * the demo); "Play again" passes Math.random to reshuffle.
 */
export function buildRound(items: readonly QuizItem[], random?: () => number): QuizItem[] {
  const order = (list: QuizItem[]) => (random ? shuffle(list, random) : [...list].sort(byTrust));
  const genuine = order(items.filter((i) => !i.isScam));
  const scams = order(items.filter((i) => i.isScam));
