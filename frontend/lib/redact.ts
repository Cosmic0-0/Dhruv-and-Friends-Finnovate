/**
 * Client-side redaction: runs in the browser BEFORE a message is sent for
 * analysis, so personal identifiers never leave the device.
 *
 * Removed:
 ...
 * English, French and Kreol, so the UI copy only promises what this does.
 *
 * Pure and dependency-free, so it's unit-testable (lib/redact.test.ts).
 * `restore()` maps placeholders back to the originals, letting the result
 * screen show the user their own text while only `redacted` was sent.
 */

export type RedactionKind = "account" | "phone" | "email";

export interface Redaction {
  kind: RedactionKind;
  /** Text that replaced the original in the redacted message. Unique per original. */
  placeholder: string;
  original: string;
}
...
interface Span {
  ...
  /** undefined = protected span (kept verbatim, blocks later matches). */
  kind?: RedactionKind;
}
...
function nearestKeyword(text: string, start: number, end: number, re: RegExp): number {
  let best = Infinity;
...
function collect(text: string, re: RegExp, spans: Span[], kind?: RedactionKind): void {
...
function classifyDigitRun(text: string, start: number, end: number): RedactionKind | null {
...
function lastFour(original: string): string {
...
export function redact(text: string): RedactionResult {
  ...
  const byOriginal = new Map<string, Redaction>();
  const redactions: Redaction[] = [];
  const counters: Record<RedactionKind, number> = { account: 0, phone: 0, email: 0 };
  const usedPlaceholders = new Set<string>();

  const placeholderFor = (kind: RedactionKind, original: string): string => {
    ...
    const r: Redaction = { kind, placeholder, original };
...
/**
 * Swap placeholders back to their originals, for display only. Works on the
 * redacted message and on server text (explanation, signal descriptions) that
 * quotes a placeholder.
 */
export function restore(text: string, redactions: readonly Redaction[]): string {
  let out = text;
  // Longest first, so "[account 2 ending 4417]" is handled before any shorter overlap.
  const ordered = [...redactions].sort((a, b) => b.placeholder.length - a.placeholder.length);
  for (const r of ordered) out = out.split(r.placeholder).join(r.original);
  return out;
}
