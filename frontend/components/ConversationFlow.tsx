    ...
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(240px,0.6fr)]">
      <section aria-label={c.thread} className="min-w-0">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h2 className="micro px-1 text-ink-muted">{c.thread} <span className="data ml-1.5 text-ink">{entries.length}</span></h2>
          {entries.length > 0 && <button type="button" disabled={loading} onClick={() => { setEntries([]); setError(null); }} className="pressable min-h-11 px-2 text-[0.9375rem] text-ink-muted disabled:opacity-40">{c.reset}</button>}
        </div>
        {entries.length === 0 && <div className="card mb-4"><p className="text-[1.0625rem] leading-[1.4375rem] text-ink-muted">{c.empty}</p></div>}
        ...
        <ol className="flex flex-col gap-3.5">
          {entries.map((entry, index) => <li key={index}><article ref={index === entries.length - 1 ? latest : undefined} className="overflow-hidden sheet">
            <div className={`flex flex-wrap items-center justify-between gap-2 border-l-[3px] px-4 py-3 ${tones[entry.response.verdict]}`}>
              <span className="micro">{c.message} {index + 1}</span>
              <span className="text-[0.9375rem] font-semibold">{c.verdicts[entry.response.verdict]}</span>
            </div>
            <p className="whitespace-pre-wrap break-words px-5 py-4 text-[1.0625rem] leading-[1.4375rem]">{entry.message}</p>
            <div className="border-t border-card-border px-5 py-4">
              ...
              <p className="mt-1.5 text-[0.9375rem] leading-5 text-ink-soft">{entry.response.explanation}</p>
            </div>
          </article></li>)}
        </ol>
        <form onSubmit={submit} className="card mt-4">
          <label htmlFor="conversation-message" className="micro block text-ink-muted">{c.add}</label>
          <textarea ref={composer} id="conversation-message" value={message} onChange={(event) => { setMessage(event.target.value); setError(null); }} disabled={loading} maxLength={MAX_MESSAGE_LENGTH} rows={5} placeholder={copy.placeholder} className="mt-2.5 w-full resize-y rounded-2xl bg-muted-surface p-3.5 text-[1.0625rem] leading-[1.4375rem] outline-none disabled:opacity-60" aria-describedby="conversation-privacy" />
      ...
      <aside className="sheet xl:sticky xl:top-6" aria-label={c.progress}>
        <details open className="group">
          <summary className="pressable flex min-h-13 cursor-pointer list-none flex-col justify-center px-5 py-3">
            <span className="micro text-ink-muted">{c.progress}</span>
            {furthest?.journey && <span className="mt-0.5 block text-[1.0625rem] text-ink">{copy.result.journey.labels[furthest.journey.currentStage]}</span>}
          </summary>
          {furthest ? <ScamJourney response={furthest} copy={copy} /> : <p className="px-5 pb-4 text-[0.9375rem] leading-5 text-ink-muted">{c.pending}</p>}
        </details>
      </aside>
    </div>
    </div>
