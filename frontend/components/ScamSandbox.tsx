...
export default function ScamSandbox() {
  ...
  return <>
    <ScreenTitle title={t("Sandbox")} back={{ href: "/app", label: copy.tabs.check }} />
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <section className="sheet min-w-0" aria-label="Educational simulation transcript">
        ...
        <div className="space-y-7 p-5 sm:p-7">
          ...
          {ended && <div className="suggestion p-5" role="status"><p className="micro text-ink-muted">{t("Simulation ended")}</p><h2 className="mt-1.5">{t("You’ve seen the pattern.")}</h2><p className="mt-2 text-[0.9375rem] leading-5 text-ink-soft">{t("Tactics encountered")}: {Array.from(new Set(turns.map(turn => turn.tactic.label.toLowerCase()))).join(", ")}. {t("In a real conversation, stop before sharing a code or sending money. Verify through the institution’s official app or a number you already trust.")}</p><div className="mt-4 flex flex-wrap gap-2.5"><button onClick={reset} className="btn-sm pressable bg-black/[0.06] text-ink">{t("Try another scenario")}</button><Link href="/app" className="btn-sm pressable bg-primary text-on-primary">{t("Check a real message")}</Link></div></div>}
          {error && <div role="alert" className="rounded-[28px] bg-danger-soft p-5"><p className="text-[1.0625rem] leading-[1.4375rem] text-ink">{error}</p>{!catalog && <button onClick={() => setAttempt(n => n + 1)} className="btn-sm pressable mt-3 bg-primary text-on-primary">{t("Reload scenarios")}</button>}</div>}
          {!ended && <button type="button" onClick={advance} disabled={busy || !playbook} className="btn pressable w-full bg-primary text-on-primary disabled:cursor-wait disabled:opacity-60">{busy ? <><span className="size-4 animate-spin rounded-full border-2 border-on-primary/30 border-t-on-primary" /><span role="status">{t("Preparing the next scenario message…")}</span></> : turns.length ? finishing ? t("Finish simulation") : t("Continue simulation") : t("Start simulation")}</button>}
