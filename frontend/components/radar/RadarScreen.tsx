...
export default function RadarScreen() {
  ...
  return (
    <DcPage label="radar" maxWidth={1180} gap={36}>
      <PageHeader
      ...
      {status === "ok" && data && (
        <>
          <div className="dc-cols-1" style={{ display: "grid", gridTemplateColumns: "minmax(0,1.3fr) minmax(0,1fr)", gap: 20, alignItems: "stretch" }}>
            <div style={{ ...card(32), padding: 28, display: "flex", flexDirection: "column", gap: 20 }} data-fx>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontSize: 13, color: "var(--dc-text3)" }}>{t.caught}</span>
                <span className="dc-mono" style={{ fontFamily: MONO, fontSize: 44, fontWeight: 700, letterSpacing: "-0.02em" }}>
                  {data.scamsCaught.toLocaleString()}
                </span>
                <span style={{ fontSize: 13, color: data.change && data.change.pct >= 0 ? "var(--dc-red)" : data.change ? "var(--dc-green)" : "var(--dc-text3)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div style={{ ...card(28), padding: 22, display: "flex", flexDirection: "column", gap: 8, flex: 1 }} data-fx>
                <span style={{ fontSize: 13, color: "var(--dc-text3)" }}>{t.mostCopied}</span>
                {data.topImpersonated ? (
                  <>
                    <span style={{ fontSize: 22, fontWeight: 600, letterSpacing: "-0.02em" }}>{data.topImpersonated.name}</span>
                    <span style={{ fontSize: 13, color: "var(--dc-text3)" }}>{fill(t.shareOf, { pct: data.topImpersonated.sharePct })}</span>
                  </>
                ) : (
              ...
              <div style={{ ...card(28), padding: 22, display: "flex", flexDirection: "column", gap: 8, flex: 1 }} data-fx>
                <span style={{ fontSize: 13, color: "var(--dc-text3)" }}>{t.rising}</span>
                {data.rising ? (
                  <>
                    <span style={{ fontSize: 22, fontWeight: 600, letterSpacing: "-0.02em" }}>{label(t.types, data.rising.scamType)}</span>
                  ...
                  <span style={{ fontSize: 15, color: "var(--dc-text3)" }}>{t.noneRising}</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="dc-split" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 20, alignItems: "start" }}>
            <section style={{ ...card(32), padding: 24, display: "flex", flexDirection: "column", gap: 14 }} data-fx>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600 }}>{t.topTypes}</h2>
                  ...
                  {data.topScamTypes.map((s) => (
                    <li key={s.scamType} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
                        ...
                        <span className="dc-mono" style={{ fontFamily: MONO, color: "var(--dc-text3)" }}>{s.pct}%</span>
                      </div>
                      <div style={{ height: 6, borderRadius: 3, background: "var(--dc-line2)" }}>
            ...
            <section style={{ ...card(32), padding: 24, display: "flex", flexDirection: "column", gap: 14 }} data-fx>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600 }}>{t.fakeLinks}</h2>
                  ...
                  {data.fakeLinks.map((l, i) => (
                    ...
                    >
                      <div style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 0 }}>
                        <span className="dc-mono" style={{ fontFamily: MONO, fontSize: 13, overflowWrap: "anywhere" }}>{l.domain}</span>
                        <span style={{ fontSize: 12, color: "var(--dc-text3)" }}>{l.imitates ? fill(t.pretends, { brand: l.imitates }) : t.imitatesUnknown}</span>
                      </div>
                      <span style={{ fontSize: 13, color: "var(--dc-text3)", whiteSpace: "nowrap" }}>{l.times === 1 ? t.once : fill(t.times, { n: l.times })}</span>
                    </li>
                  ))}
                </ul>
              )}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {allZero ? (
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: data.series.length > 20 ? 2 : 5, height: 140 }}>
          {data.series.map((b) => (
            <div
              key={b.start}
              ...
              style={{
                ...
                borderRadius: 3,
                background: b.scams > 0 ? "var(--dc-accent)" : "var(--dc-line2)",
              }}
            />
          ))}
        </div>
