...
// in a PDF/DOCX plus its text through the same runPipeline(). The type is
// decided by magic bytes only (`fileName` is display-only on the client and
// ignored here); parsing runs in a heap-capped worker with a hard timeout;
// the file, its text and its previews are never stored, and the LLM only
// ever sees the redacted text.
