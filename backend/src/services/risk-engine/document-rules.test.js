...
test("floor: a pasted transparent/upscaled image or typed-on text on a document claiming a known institution -> HIGH", () => {
  for (const s of [doc("DOC-04", "transparent_overlay"), doc("DOC-04", "resolution_mismatch"), doc("DOC-05")]) {
    ...
    assert.equal(r.level, "high", `${s.code}:${s.metadata.variant}`);
    assert.ok(ids(r).includes("FLOOR-DOC-FORGED-INSTITUTION"));
    assert.equal(r.decision, "do_not_pay");
    // Without a claimed institution the same fact stays ELEVATED.
    assert.equal(score([s]).level, "elevated");
  }
  // The variant filter keeps weak image findings out of the floor.
