// Ported from frontend/lib/redact.test.ts - keep assertions in sync by hand.
import { test } from "node:test";
import assert from "node:assert/strict";
import { redact } from "./index.js";

test("account numbers of 9+ digits keep only the last 4", () => {
  const { redacted } = redact("Your account 000123454417 has been blocked.");
...
test("email local part is redacted, domain kept for lookalike checks", () => {
  const { redacted } = redact("Reply to jean.pierre@mcb-secure-login.com urgently");
...
test("URLs and domains are never redacted, even with digits in them", () => {
  ...
  assert.equal(redact(input).redacted, input);
});
...
test("OTP codes near a code keyword are kept", () => {
  for (const input of [
    ...
    assert.equal(redact(input).redacted, input, input);
  }
});
