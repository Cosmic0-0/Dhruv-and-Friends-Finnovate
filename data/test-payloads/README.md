...
`../kreol-dataset/scam-corpus.csv` (owned by Joshua) and wasn't duplicated
...
- **Source of truth stays the CSV.** `kr.json` is a point-in-time snapshot,
  not generated by a script. If Joshua's `owner_reviewed` set changes,
...
- **`verdict` is inferred, not sourced.** The CSV has no verdict field, only
  ...
  - **Known gap:** that's a 20/1/3 scam/suspicious/safe split, more
    ...
    without Joshua's review would be worse than the skew. If he adds more
